...
#define MACHINE_MAX_PAGES MACHINE_MMSIZE >> PAGE_SHIFT
struct bootmm bmm;//bmm system
...
uint align_func(uint value, uint align){
    value += align - 1;
    value &= ~(align - 1);
    return value;
}

void set_mminfo(struct bootmm_info *info, uint start, uint end, uint type) {
    info->start = start;
    info->end = end;
    info->type = type;
...
uchar bootmmmap[MACHINE_MAX_PAGES];
...
uchar *find_pages(uint page_cnt, uint s_pfn, uint e_pfn, uint align_pfn) {//align_pfn = 1
    ...
    for (index = s_pfn; index < e_pfn; ) {
        ...
        if (count == 0) {  // count = 0 means found
            ...
            index = temp + align_pfn;  // there will be no possible memory space
                                      // to be allocated before temp
        }
    }
    return 0;
}
...
uchar *bootmm_alloc_pages(uint size, uint type, uint align) {
    ...
    size = align_func(size,1 << PAGE_SHIFT);
    ...
    if (res) {
        if(insert_mminfo(&bmm, (uint)res, (uint)res + size - 1, type) != 0)
            return res;
        else
            return 0;//无法插入
    }
